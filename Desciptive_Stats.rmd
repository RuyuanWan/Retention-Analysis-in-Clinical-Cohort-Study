...
LTV_disease<- Users_LTV %>%
           left_join(Users)%>%
           filter(InitPDDiag== "1" & InitPDDiagAge >= 30)%>%
           mutate(PDstage = (age-InitPDDiagAge) )#%>%  
...
LTV_disease_factor <- LTV_disease %>%
        mutate(PD_stage_group = case_when(PDstage<=2 ~ "(0,2]",PDstage>2 & PDstage <=5 ~ "(2,5]", 
...
cor.test(Users_Age_PDstage$age,Users_Age_PDstage$PDstage)

wilcox.test(PDstage_male$age,PDstage_female$age  )

wilcox.test(Age_male$age,Age_female$age  )
